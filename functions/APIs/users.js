...
deleteImage = async imageName => {
    ...
    try {
        const imgDelete = await bucket.file(path).delete();
        console.info(`Deleted image ${imgDelete}`);
        // return imgDelete;
    } catch (err) {
        console.error(err);
    }
...
exports.uploadProfilePhoto = (request, response) => {
    ...
    console.info('start profile upload....');
    busboy.on('file', (fieldname, file, filename, encoding, mimetype) => {
        if (mimetype !== 'image/png' && mimetype !== 'image/jpeg') {
            return response.status(400).json({ error: 'Wrong file type submited' });
        }
        const imageExtention = filename.split('.')[filename.split('.').length - 1];
        imageFileName = `${request.user.username}.${imageExtention}`;
        const filePath = path.join(os.tmpdir(), imageExtention);
    ...
    console.info(
        `imageToBeUploaded.filePath=${JSON.stringify(
      imageToBeUploaded.filePath
    )} :: imageToBeUploaded.mimetype=${JSON.stringify(
      imageToBeUploaded.mimetype
    )}`
    );
    busboy.on('finish', async() => {
        try {
            console.log('uplodedImage');
            const uplodedImage = await admin
                .storage()
                .bucket()
            ...
            const updatedImageUrl = await db
                .doc(`/users/${request.user.username}`)
                .update({ imageUrl });
            console.info(
                `Image updated - ${JSON.stringify(uplodedImage)} , ${JSON.stringify(
          updatedImageUrl
        )}`
            );
